...
import microapp.ticket.config.KafkaSseConsumer;
import microapp.ticket.config.KafkaSseProducer;
...
@RestController
@RequestMapping("/api/ticket-kafka")
public class TicketKafkaResource {

    private final Logger log = LoggerFactory.getLogger(TicketKafkaResource.class);

    private final MessageChannel output;
    private Sinks.Many<Message<String>> sink = Sinks.many().unicast().onBackpressureBuffer();

    public TicketKafkaResource(@Qualifier(KafkaSseProducer.CHANNELNAME) MessageChannel output) {
        this.output = output;
    }
